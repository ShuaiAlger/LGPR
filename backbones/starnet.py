...
@register_model
def starnet_ss(pretrained=False, **kwargs):
    return StarNet(24, [1, 1, 2, 3], 3, **kwargs)

import torch.nn.functional as F
import time


class StarNetBuilder(nn.Module):
    AVAILABLE_MODELS = {
        "starnet": starnet_ss
    }
    def __init__(
        self,
        backbone_name="starnet",
        pretrained=True,
        ...
        self.starnet = self.AVAILABLE_MODELS[self.backbone_name]()

        self.out_channels = 64 * 3


    def forward(self, x):
        x = self.starnet.stem(x)
