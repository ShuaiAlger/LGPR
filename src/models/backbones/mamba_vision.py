...
class MambaVisionBuilder(nn.Module):
    AVAILABLE_MODELS = {
        "mamba_vision_T": mamba_vision_T,
        "mamba_vision_T2": mamba_vision_T2,
        "mamba_vision_S": mamba_vision_S,
        "mamba_vision_B": mamba_vision_B,
        "mamba_vision_L": mamba_vision_L,
        "mamba_vision_L2": mamba_vision_L2
    }
    ...
    def __init__(
        ...
        self.mambavision = self.AVAILABLE_MODELS[self.backbone_name](pretrained=self.pretrained)


        # freeze the first blocks, keep only the last num_unfrozen_blocks trainable
        for i in range(len(self.mambavision.levels) - self.num_unfrozen_blocks):
            self.mambavision.levels[i].requires_grad_(False)
        
        # print("self.mambavision.levels[-2].parameters.shape : ")

        self.out_channels = 512

    def forward(self, x):
        x = self.mambavision.patch_embed(x)
        with torch.no_grad():
            for l in range(len(self.mambavision.levels) - self.num_unfrozen_blocks):
                x = self.mambavision.levels[l](x)

        # for l in range(len(self.mambavision.levels) - self.num_unfrozen_blocks, len(self.mambavision.levels) - 1):
        #         x = self.mambavision.levels[l](x)
        return x 
