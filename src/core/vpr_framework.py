...
class VPRFramework(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if isinstance(model_output, tuple) or isinstance(model_output, list):
            descriptors = model_output[0]
            sidelength = int(np.sqrt(model_output[1][0][0, 0].shape[-1]))
            attention_display = model_output[1][0][0, 0].reshape(sidelength, sidelength).detach().cpu().numpy()

            attention_display = np.uint8(255*(attention_display - attention_display.min())/(attention_display.max() - attention_display.min()))
            attention_display = cv2.applyColorMap(attention_display, cv2.COLORMAP_JET)
            index_save = self.batch_idx
            cv2.imwrite("/media/shuai/Correspondence/explore/OpenJKL/display/"+str(index_save)+"boq_attention0.png", attention_display)

            sidelength = int(np.sqrt(model_output[1][1][0, 0].shape[-1]))
            attention_display = model_output[1][1][0, 0].reshape(sidelength, sidelength).detach().cpu().numpy()
            attention_display = np.uint8(255*(attention_display - attention_display.min())/(attention_display.max() - attention_display.min()))
            attention_display = cv2.applyColorMap(attention_display, cv2.COLORMAP_JET)
            index_save = self.batch_idx
            cv2.imwrite("/media/shuai/Correspondence/explore/OpenJKL/display/"+str(index_save)+"boq_attention1.png", attention_display)
        ...
        for dataloader_idx, descriptors_list in self.validation_step_outputs.items():
            ...
            if self.trainer.fast_dev_run:
                ...
                recalls_dict = utils.compute_recall_performance(
                        ...
                        k_values=[1, 5, 10, 15, 20, 25, 50, 100]
                )
                recalls_log = {
