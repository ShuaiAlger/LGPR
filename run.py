...
def load_config(config_path='/media/shuai/Correspondence/explore/OpenVPRLab/config/boq_dinov2.yaml'):
...
def train(config):
    ...
    checkpoint_cb = ModelCheckpoint(
        monitor="TJPark/R1",
        filename="epoch({epoch:02d})_step({step:04d})_R1[{TJPark/R1:.4f}]_R5[{TJPark/R5:.4f}]",
        auto_insert_metric_name=False,
        save_weights_only=False,
        save_top_k=3,
        mode="max",
